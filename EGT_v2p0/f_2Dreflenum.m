...
function f_2Dreflenum(Struct_Point, color, distance);
    if nargin<1
        display('EGT error: function "f_2Dreflenum" needs 1 parameter at least');
    elseif nargin<2
        color = 'r*';
        distance = 0;
    elseif nargin<3
        distance = 0;
    elseif nargin>3
        display('EGT warning: too much input parameters in "f_2Dreflenum"!');
    end;
    
    for index = 1:size(Struct_Point, 2)
        point = Struct_Point(index).point;
        label = strcat(num2str(Struct_Point(index).number));
        plot(point(1), point(2), color);
        text(point(1)+distance, point(2)+distance, label);
    end