function [E,ga]=funmatMestTorrPanCameras(M,n,error)
%Ogni colonna di M contiene le coordinate negli specchi di uno dei punti della scena 3D 
%M=[XMa; 
%   XMd]
...
if (size(M,1)~=6) | (size(M,2)<8),
 ...
 %Crea la matrice A tale che Ae=0
	for i=1:size(M,2),
        A=[A;(kron(M(1:3,i),M(4:6,i)))']; %<== COntrolla questa!!!
