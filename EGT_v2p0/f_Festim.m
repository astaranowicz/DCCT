...
%                    = 6,  "Torr's method (Robust)"
% Author:
%     Gian Luca Mariottini 
% Last update:
%     March 2008
function F=f_Festim(U,Up,algorithm,roundf,error_estim_F_Torr);
global ga;
...
%[f,f_sq_errors] = torr_estimateF(Mn(:,:)', 1, [], 'non_linear',1,f0);
      ...
      a=f_roundn(a,roundf); %GLM
      

      F0=unstkc(a,3,3);
      F=F';
          
      F=f_forcerank2(F); %GLM
...
elseif algorithm==4,
    global inlier_index
      ...
      % Parameters
      %global error_estim_F_Torr;
      %error_estim_F_Torr=10^-5; % Keep under "-5"     
      iterations_Torr=1500;      
      [F,ga] = f_FestimM_byTorr(Mn,iterations_Torr,error_estim_F_Torr,Tp,T);
      Thresh_ga_outli=((min(ga)*(1/2)+median(ga))/(1+(1/2)^2)/0.6475);
      ind_inliers=find(abs(ga)<=Thresh_ga_outli);
      %keyboard
      if length(ind_inliers)<8,
        Thresh_ga_outli=(min(ga)+median(ga))/2;
        ind_inliers=find(abs(ga)<=Thresh_ga_outli);
      end 
      inlier_index=ind_inliers;
      ...
      % Final F_estimation
      [F,ga]=f_FestimM_byTorr(Mn(:,inlier_index), iterations_Torr, error_estim_F_Torr, Tp, T); 
...
elseif algorithm==5, 
    global inlier_index;
      ...
      %% F estimation
        b=8;%7; %bucket
        p=0.999; %Probability that there is an F without outliers
        r=0.15; %Outlier_ratio
        [F,ga]=funmatLMedSeig(Mn,b,p,r); 
      %% Outliers removal              
          %Thresh_ga_outli=(min(ga)*(1/4)+median(ga))/(1+(1/4));
          Thresh_ga_outli=(min(ga)*(1/4)+median(ga))/(1+(1/4)^2);
          % Globalize threshold to be recalled by calling routine
          global threshold
          threshold=Thresh_ga_outli;
          global ga;
          ind_inliers=find(abs(ga)>=Thresh_ga_outli);
          if length(ind_inliers)<=b,
      ...
      %n=norm(F,'fro');
        %if n~=0,
            %  F=F./norm(F); %is better with the normalization
        %end
...
function [F,g]=f_FestimM_byTorr(M,n,error,Td,Ta);
   ...
   U=[];
   %Up(1,i)*U(1,i) , Up(1,i)*U(2,i) , Up(1,i) , Up(2,i)*U(1,i) , Up(2,i)*U(2,i) , Up(2,i) , U(1,i) , U(2,i) , 1
   for i=1:size(M,2),
...
%            U(i,:)
%            g
%            display('ce un elemento ad infinito!')
%            pause
%        end
       U2(i,:)=U(i,:).*w2(i);         
     end       
   ...
   for i=1:size(M,2),
      ...
      for i=1:size(M,2),
         if (abs(dt(i))<3*s_i),%&(abs(dt(i))>0), 
             g(i)=s_i/abs(dt(i));
         elseif (abs(dt(i))<s_i),%&(abs(dt(i))>0), 
             g(i)=1;
         else g(i)=0; end
      end
   end
